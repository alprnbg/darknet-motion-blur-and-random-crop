...
int fill_truth_detection(const char *path, int num_boxes, int truth_size, float *truth, int classes, int flip, float dx, float dy, float sx, float sy,
    int net_w, int net_h, box_label* boxes)
{
    char labelpath[4096];
    replace_image_to_label(path, labelpath);
    ...
    // CHANGE BOXES ACC. TO NEW LABELS
    count = 1;

    /*
    printf("%f %f %f %f \n", boxes[0].left, boxes[0].right, boxes[0].top, boxes[0].bottom);
    printf("%f %f %f %f \n", boxes[0].x, boxes[0].y, boxes[0].w, boxes[0].h);
    printf("%d %d\n\n", boxes[0].id, boxes[0].track_id);
    */
...
data load_data_detection(int n, char **paths, int m, int w, int h, int c, int boxes, int truth_size, int classes, int use_flip, int use_gaussian_noise, int use_blur, int use_mixup,
    float jitter, float resize, float hue, float saturation, float exposure, int mini_batch, int track, int augment_speed, int letter_box, int mosaic_bound, int contrastive, int contrastive_jit_flip, int contrastive_color, int show_imgs, float motionBlurRate, int motionBlurX, int motionBlurY)
{

    if (random_float() > motionBlurRate){
        motionBlurX = 0;
        motionBlurY = 0;
    }
    ...
    for (i_mixup = 0; i_mixup <= use_mixup; i_mixup++) {
        ...
        for (i = 0; i < n; ++i) {
            ...
            char random_crop_labelpath[4096];
            replace_image_to_label(filename, random_crop_labelpath);
            int random_crop_label_count = 0;
            box_label *random_crop_boxes = read_boxes(random_crop_labelpath, &random_crop_label_count);
            int selected_ball = rand_int(0, random_crop_label_count-1);

            int random_crop_xmin = (int)(random_crop_boxes[selected_ball].left * ow);
            int random_crop_ymin = (int)(random_crop_boxes[selected_ball].top * oh);
            int random_crop_xmax = (int)(random_crop_boxes[selected_ball].right * ow);
            int random_crop_ymax = (int)(random_crop_boxes[selected_ball].bottom *oh);

            int random_crop_x = random_crop_xmin;
            int random_crop_y = random_crop_ymin;
            int random_crop_w = random_crop_xmax-random_crop_xmin;
            int random_crop_h = random_crop_ymax-random_crop_ymin;
            int random_crop_radius = random_crop_w < random_crop_h ? random_crop_h : random_crop_w;
            int random_crop_rand_x = rand_int(0+random_crop_radius, CROP_SIZE-random_crop_radius-1);
            int random_crop_rand_y = rand_int(0+random_crop_radius, CROP_SIZE-random_crop_radius-1);

            int random_crop_crop_x_0 = random_crop_x - random_crop_rand_x;
            int random_crop_crop_x_1 = random_crop_x - random_crop_rand_x + CROP_SIZE;

            int random_crop_crop_y_0 = random_crop_y - random_crop_rand_y;
            int random_crop_crop_y_1 = random_crop_y - random_crop_rand_y + CROP_SIZE;

            if (random_crop_crop_x_0 < 0){
                random_crop_crop_x_0 = 0;
                random_crop_crop_x_1 = CROP_SIZE;
            }
            else if (random_crop_crop_x_1 > ow-1){
                random_crop_crop_x_0 = ow - 1 - CROP_SIZE;
                random_crop_crop_x_1 = ow - 1;
            }

            if (random_crop_crop_y_0 < 0){
                random_crop_crop_y_0 = 0;
                random_crop_crop_y_1 = CROP_SIZE;
            }
            else if (random_crop_crop_y_1 > oh-1){
                random_crop_crop_y_0 = oh - 1 - CROP_SIZE;
                random_crop_crop_y_1 = oh - 1;
            }

            int crop_indices[4] = {random_crop_crop_x_0, random_crop_crop_y_0, random_crop_crop_x_1-random_crop_crop_x_0, random_crop_crop_y_1-random_crop_crop_y_0};

            /*
            printf("%d %d %d %d\n\n",random_crop_crop_x_0, random_crop_crop_y_0, random_crop_crop_x_1-random_crop_crop_x_0, random_crop_crop_y_1-random_crop_crop_y_0);
            printf("%d %d \n\n", random_crop_w, random_crop_h);

            printf("%f %f %f %f \n", random_crop_boxes[selected_ball].left, random_crop_boxes[selected_ball].right, random_crop_boxes[selected_ball].top, random_crop_boxes[selected_ball].bottom);
            printf("%f %f %f %f \n", random_crop_boxes[selected_ball].x, random_crop_boxes[selected_ball].y, random_crop_boxes[selected_ball].w, random_crop_boxes[selected_ball].h);
            printf("%d %d\n\n", random_crop_boxes[selected_ball].id, random_crop_boxes[selected_ball].track_id);
            */

            random_crop_boxes[0].left = (float)((int)(random_crop_boxes[selected_ball].left * ow) - random_crop_crop_x_0) / CROP_SIZE;
            random_crop_boxes[0].top = (float)((int)(random_crop_boxes[selected_ball].top * oh) - random_crop_crop_y_0) / CROP_SIZE;
            random_crop_boxes[0].bottom = random_crop_boxes[0].top + (float)random_crop_h/CROP_SIZE;
            random_crop_boxes[0].right = random_crop_boxes[0].left + (float)random_crop_w/CROP_SIZE;
            random_crop_boxes[0].w = (float)random_crop_w/CROP_SIZE;
            random_crop_boxes[0].h = (float)random_crop_h/CROP_SIZE;
            random_crop_boxes[0].x = (random_crop_boxes[0].left + random_crop_boxes[0].right) / 2;
            random_crop_boxes[0].y = (random_crop_boxes[0].top + random_crop_boxes[0].bottom) / 2;
            random_crop_boxes[0].id = random_crop_boxes[selected_ball].id;
            random_crop_boxes[0].track_id = random_crop_boxes[selected_ball].track_id;

            /*
            printf("%f %f %f %f \n", random_crop_boxes[0].left, random_crop_boxes[0].right, random_crop_boxes[0].top, random_crop_boxes[0].bottom);
            printf("%f %f %f %f \n", random_crop_boxes[0].x, random_crop_boxes[0].y, random_crop_boxes[0].w, random_crop_boxes[0].h);
            printf("%d %d\n\n", random_crop_boxes[0].id, random_crop_boxes[0].track_id);
            */

            for (int j = 1; j < random_crop_label_count; j++){
                random_crop_boxes[j].left = 0;
                random_crop_boxes[j].top = 0;
                random_crop_boxes[j].bottom = 0;
                random_crop_boxes[j].right = 0;
                random_crop_boxes[j].w = 0;
                random_crop_boxes[j].h = 0;
                random_crop_boxes[j].x = 0;
                random_crop_boxes[j].y = 0;
                random_crop_boxes[j].id = 0;
                random_crop_boxes[j].track_id = 0;
            }
            ...
            int min_w_h = fill_truth_detection(filename, boxes, truth_size, truth, classes, flip, dx, dy, 1. / sx, 1. / sy, w, h, random_crop_boxes);  // GIVE NEW LABELS !!!
            ...
            /*
            printf("%s \n", filename);
            for(int i = 0; i < truth_size * boxes; i++){
                printf("%f ",truth[i]);
            }
            */
            image ai = image_data_augmentation(src, w, h, pleft, ptop, swidth, sheight, flip, dhue, dsat, dexp,
                gaussian_noise, blur, boxes, truth_size, truth, crop_indices, motionBlurX, motionBlurY);
    ...
    float jitter, float resize, float hue, float saturation, float exposure, int mini_batch, int track, int augment_speed, int letter_box, int mosaic_bound, int contrastive, int contrastive_jit_flip, int contrastive_color, int show_imgs, float motionBlurRate, int motionBlurX, int motionBlurY)
{
    ...
    for (i_mixup = 0; i_mixup <= mixup; i_mixup++) {
        ...
        for (i = 0; i < n; ++i) {
            ...
            box_label* dummy;
            fill_truth_detection(filename, boxes, truth_size, truth, classes, flip, dx, dy, 1. / sx, 1. / sy, w, h, dummy);
...
void *load_thread(void *ptr)
{
    ...
    if (a.type == OLD_CLASSIFICATION_DATA){
        ...
        *a.d = load_data_detection(a.n, a.paths, a.m, a.w, a.h, a.c, a.num_boxes, a.truth_size, a.classes, a.flip, a.gaussian_noise, a.blur, a.mixup, a.jitter, a.resize,
            a.hue, a.saturation, a.exposure, a.mini_batch, a.track, a.augment_speed, a.letter_box, a.mosaic_bound, a.contrastive, a.contrastive_jit_flip, a.contrastive_color, a.show_imgs, a.motionBlurRate, a.motionBlurX, a.motionBlurY);
