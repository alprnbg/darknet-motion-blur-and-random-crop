...
/*
extern "C" void* crop_image_cv(mat_cv* mat, int x, int y, int w, int h){
    cv::Mat* img = (cv::Mat *) mat;
    cv::Rect crop_rect(x, y, w, h);
    cv::Mat croppedImg = (*img)(crop_rect);
    mat = (mat_cv*) &croppedImg;
}
*/
...
extern "C" image image_data_augmentation(mat_cv* mat, int w, int h,
    ...
    int gaussian_noise, int blur, int num_boxes, int truth_size, float *truth, int* cropIndices, int motionBlurX, int motionBlurY)
{
    image out;
    try {
        cv::Mat raw_img = *(cv::Mat *)mat;

        //cv::namedWindow("org", cv::WINDOW_NORMAL);
        //cv::imshow("org", raw_img);

        cv::Mat img = raw_img(cv::Rect(cropIndices[0], cropIndices[1], cropIndices[2], cropIndices[3]));
        ...
        //cv::imshow("before", sized);

        if (motionBlurY){

            cv::Mat kernel_v;
            int kernel_size_y = motionBlurY;
            int mid = (kernel_size_y-1)/2;
            kernel_v = cv::Mat::zeros(kernel_size_y, kernel_size_y, CV_32F);

            for(int i = 0; i < kernel_size_y; i++)
                kernel_v.at<float>(i, mid) = 1.0/kernel_size_y;
            ...
            delta = 0;
            ddepth = -1;
            cv::filter2D(sized, sized, ddepth , kernel_v, anchor, delta, cv::BORDER_DEFAULT);
        }

        if (motionBlurX){

            cv::Mat kernel_h;
            int kernel_size_x = motionBlurX;
            int mid = (kernel_size_x-1)/2;
            kernel_h = cv::Mat::zeros(kernel_size_x, kernel_size_x, CV_32F);

            for(int i = 0; i < kernel_size_x; i++)
                kernel_h.at<float>(mid, i) = 1.0/kernel_size_x;
            ...
            delta = 0;
            ddepth = -1;
            cv::filter2D(sized, sized, ddepth , kernel_h, anchor, delta, cv::BORDER_DEFAULT);
        }

        //cv::imshow("after", sized);
        //cv::waitKey(0);
